...
movable_function<ReturnType(Args...) const> constCastFunction(
        movable_function<ReturnType(Args...)>&&) noexcept;
...
template <class T>
struct NotFunction<movable_function<T>> : std::false_type {};
...
struct FunctionTraits<ReturnType(Args...)> {
    ...
    ReturnType operator()(Args... args) {
        auto& fn = *static_cast<movable_function<ReturnType(Args...)>*>(this);
        return fn.call_(fn.data_, static_cast<Args&&>(args)...);
    }

    class SharedProxy {
        std::shared_ptr<movable_function<ReturnType(Args...)>> sp_;

    public:
        explicit SharedProxy(movable_function<ReturnType(Args...)>&& func)
                : sp_(std::make_shared<movable_function<ReturnType(Args...)>>(
                std::move(func))) {}
...
struct FunctionTraits<ReturnType(Args...) const> {
    ...
    ReturnType operator()(Args... args) const {
        auto& fn = *static_cast<const movable_function<ReturnType(Args...) const>*>(this);
        return fn.call_(fn.data_, static_cast<Args&&>(args)...);
    }

    class SharedProxy {
        std::shared_ptr<movable_function<ReturnType(Args...) const>> sp_;

    public:
        explicit SharedProxy(movable_function<ReturnType(Args...) const>&& func)
                : sp_(std::make_shared<movable_function<ReturnType(Args...) const>>(
                std::move(func))) {}
        ReturnType operator()(Args&&... args) const {
            return (*sp_)(static_cast<Args&&>(args)...);
        }
...
template <typename FunctionType>
class movable_function final : private detail::function::FunctionTraits<FunctionType> {
    ...
    using Target = boost::variant<boost::blank, std::type_index>;
    ...
    friend Traits;
    friend movable_function<typename Traits::ConstSignature> di::tools::constCastFunction<>(
            movable_function<typename Traits::NonConstSignature>&&) noexcept;
    friend class movable_function<typename Traits::OtherSignature>;

    template <typename Fun>
    movable_function(Fun&& fun, SmallTag) noexcept {
        ...
        if (!detail::function::isNullPtrFn(fun)) {
            ...
            target_ = std::type_index(typeid(Fun));
        }
    }

    template <typename Fun>
    movable_function(Fun&& fun, HeapTag) {
        using FunT = typename std::decay<Fun>::type;
    ...
    movable_function(movable_function<Signature>&& that, CoerceTag)
            : movable_function(static_cast<movable_function<Signature>&&>(that), HeapTag{}) {}

    movable_function(
            movable_function<typename Traits::OtherSignature>&& that,
            CoerceTag) noexcept {
...
public:
     ...
     * Move constructor
     */
    movable_function(movable_function&& that) noexcept {
        that.exec_(Op::MOVE, &that.data_, &data_);
        std::swap(call_, that.call_);
        std::swap(exec_, that.exec_);
        std::swap(target_, that.target_);
     ...
     */
    /* implicit */ movable_function(std::nullptr_t) noexcept {}
    ...
    /* implicit */ movable_function(Fun&& fun) noexcept(
    IsSmall<Fun>::value && noexcept(FunT(std::declval<Fun>())))
            : movable_function(static_cast<Fun&&>(fun), IsSmall<Fun>{}) {}
#else
    // FIXME: The above doesn't compile on GCC 6.2, simplified version instead
    template <
            typename Fun,
            typename = typename Traits::template ResultOf<Fun>>
    /* implicit */ movable_function(Fun&& fun) noexcept
    ...
    template <
            typename Signature,
            typename = typename Traits::template ResultOf<movable_function<Signature>>>
    movable_function(movable_function<Signature>&& that) noexcept(
    noexcept(movable_function(std::move(that), CoerceTag{})))
            : movable_function(std::move(that), CoerceTag{}) {}
    ...
    template <
            ...
            typename = decltype(movable_function(std::mem_fn((Member Class::*)0)))>
    /* implicit */ movable_function(Member Class::*ptr) noexcept {
        if (ptr) {
            *this = std::mem_fn(ptr);
        }
    }
     ...
     */
    movable_function& operator=(movable_function&& that) noexcept {
        // Q: Why is is safe to destroy and reconstruct this object in place?
        // A: Two reasons: First, `movable_function` is a final class, so in doing this
        ...
        // "moved" into itself, which is a no-op for a default-constructed movable_function.
        movable_function();
        ::new (this) movable_function(std::move(that));
        return *this;
    }
     ...
     * selected by overload resolution when `fun` is not a compatible function.
     */
    template <typename Fun, typename = decltype(movable_function(std::declval<Fun>()))>
    movable_function& operator=(Fun&& fun) noexcept(
    noexcept(/* implicit */ movable_function(std::declval<Fun>()))) {
        // Doing this in place is more efficient when we can do so safely.
        if (noexcept(/* implicit */ movable_function(std::declval<Fun>()))) {
            // Q: Why is is safe to destroy and reconstruct this object in place?
            // A: See the explanation in the move assignment operator.
            movable_function();
            ::new (this) movable_function(static_cast<Fun&&>(fun));
        } else {
            // Construct a temporary and (nothrow) swap.
            movable_function(static_cast<Fun&&>(fun)).swap(*this);
        }
        return *this;
    }
    ...
    template <
            typename Signature,
            typename = typename Traits::template ResultOf<movable_function<Signature>>>
    movable_function& operator=(movable_function<Signature>&& that) noexcept(
    noexcept(movable_function(std::move(that)))) {
        return (*this = movable_function(std::move(that)));
    }

    /**
     * Clears this `Function`.
     */
    movable_function& operator=(std::nullptr_t) noexcept {
        return (*this = movable_function());
    }
    ...
    -> decltype(operator=(std::mem_fn(ptr))) {
        return ptr ? (*this = std::mem_fn(ptr)) : (*this = movable_function());
    }
     ...
     */
    void swap(movable_function& that) noexcept {
        std::swap(*this, that);
    }
    ...
    std::type_index target_type() {
        if (target_.type() == typeid(boost::blank)){
            return typeid(void);
        }

        return boost::get<std::type_index>(target_);
    }
};

template <typename FunctionType>
void swap(movable_function<FunctionType>& lhs, movable_function<FunctionType>& rhs) noexcept {
    lhs.swap(rhs);
}

template <typename FunctionType>
bool operator==(const movable_function<FunctionType>& fn, std::nullptr_t) {
    return !fn;
}

template <typename FunctionType>
bool operator==(std::nullptr_t, const movable_function<FunctionType>& fn) {
    return !fn;
}
...
template <typename ReturnType, typename... Args>
movable_function<ReturnType(Args...) const> constCastFunction(
        movable_function<ReturnType(Args...)>&& that) noexcept {
    return movable_function<ReturnType(Args...) const>{std::move(that),
                                               detail::function::CoerceTag{}};
}

template <typename ReturnType, typename... Args>
movable_function<ReturnType(Args...) const> constCastFunction(
        movable_function<ReturnType(Args...) const>&& that) noexcept {
...
// helpers added to Facebook folly function
namespace di { namespace tools {
